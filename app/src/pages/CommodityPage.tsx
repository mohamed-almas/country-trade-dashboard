...
function TradeCorridorsTable({
  ...
  if (!corridors || corridors.length === 0) {
    return (
      <div className="border p-5 text-center" style={card}>
        <p className="text-sm" style={tm}>No trade corridor data available for {commodityName}</p>
      </div>
    );
  }
  ...
  return (
    <div className="border" style={card}>
      <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
        <h2 className="text-base font-semibold font-outfit" style={tp}>
          Top 20 Trade Corridors — {commodityName} ({year})
        </h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: `1px solid var(--border)`, backgroundColor: 'var(--bg-primary)' }}>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={tm}>#</th>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={tm}>Exporter</th>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={tm}>Importer</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide" style={tm}>
                {metric === 'value' ? 'Trade Value' : 'Trade Volume'}
              </th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide" style={tm}>Share of Top 20</th>
            </table>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.exporter}-${row.importer}-${row.rank}`} style={{ borderBottom: '1px solid var(--border)' }}>
                <td className="px-4 py-3 text-xs tabular-nums" style={tm}>{row.rank}</td>
                <td className="px-4 py-3 whitespace-nowrap" style={tp}><CorridorCountryCell name={row.exporter} /></td
                <td className="px-4 py-3 whitespace-nowrap" style={tp}><CorridorCountryCell name={row.importer} /></td
                <td className="px-4 py-3 text-right font-medium tabular-nums" style={tp}>{formatCompactNumber(row.value)}</td
                <td className="px-4 py-3 text-right tabular-nums text-xs" style={ts}>{row.share.toFixed(2)}%</td
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="min-h-screen pt-14" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          <div className="border p-5 md:p-6" style={card}>
            <SectionHeader title={`Top ${showTopCountriesExport ? 'Exporting' : 'Importing'} Countries — ${displayLabel} (${year})`} action={<div className="flex gap-2"><button onClick={() => setShowTopCountriesExport(true)} className="px-3 py-1.5 text-xs font-semibold transition-colors" style={{ backgroundColor: showTopCountriesExport ? 'var(--accent)' : 'var(--bg-tertiary)', color: showTopCountriesExport ? 'var(--accent-text)' : 'var(--text-secondary)', border: `1px solid ${showTopCountriesExport ? 'var(--accent)' : 'var(--border)'}` }}>Exports</button><button onClick={() => setShowTopCountriesExport(false)} className="px-3 py-1.5 text-xs font-semibold transition-colors" style={{ backgroundColor: !showTopCountriesExport ? 'var(--accent)' : 'var(--bg-tertiary)', color: !showTopCountriesExport ? 'var(--accent-text)' : 'var(--text-secondary)', border: `1px solid ${!showTopCountriesExport ? 'var(--accent)' : 'var(--border)'}` }}>Imports</button></div>} />
        ...
        {/* Top 20 Trade Corridors Table */}
        <TradeCorridorsTable
          corridors={tradeCorridors}
          metric={metric}
          year={year}
          commodityName={displayLabel}
        />
